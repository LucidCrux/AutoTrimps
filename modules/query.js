...
function getEnoughHealth(enemyDamage) {
    ...
    const FORMATION_MOD_1 = game.upgrades.Dominance.done ? 2 : 1;  //Divides baseHealth, baseBlock
    var actualHealth = baseHealth/FORMATION_MOD_1;
    var potentialDamage = enemyDamage - baseBlock/FORMATION_MOD_1 > 0 ? enemyDamage - baseBlock/FORMATION_MOD_1 : enemyDamage * pierceMod;
    return (actualHealth > MODULES["automaps"].numHitsSurvived * potentialDamage) || (actualHealth > potentialDamage && actualHealth - (potentialDamage * getBreedTime() / combatSpeed) > 0);
}

//Determines if damage is high enough for one shot kills
function getEnoughDamage(enemyHealth) {
  const FORMATION_MOD_1 = game.upgrades.Dominance.done ? 4 : 1;  //Multiplies baseDamage 
  return (baseDamage * FORMATION_MOD_1 > enemyHealth);
}
